...
def train_model(train_paths, val_paths, output_best_params, batch_size):
    ...
    train_dataset, train_steps = create_train_val_datasets(file_paths=train_paths["image_path"].to_numpy(),
                                              ...
                                              metadata = train_paths.iloc[:, 2:].to_numpy(),
                                              # access the paths to labels of training images from train_paths df
                                              batch_size= batch_size,
                                              num_epochs = int(best_params["num_epochs"][0]),
    ...
    model, base_model =  design_model_transfer_phase1(int(best_params["img_size"][0]),
                          int(best_params["num_channels"][0]),
                          float(best_params["dropout_val"][0]),
                          ast.literal_eval(best_params["num_dense_units"][0]),
                          best_params["activation_dense"][0],
                          float(best_params["l2_reg_dense"][0]),
                          int(best_params["nodes_output"][0]),
                          best_params["activation_output"][0],
                          float(best_params["learning_rate"][0]),
                          float(best_params["alpha"][0]),
                          float(best_params["gamma"][0]))
